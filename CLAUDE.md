...
- **`tts/`** — Text-to-speech via OpenAI-compatible TTS API.
  ...
  - Supported providers: `"openai"` (default, model `tts-1`) and `"groq"` (model `canopylabs/orpheus-v1-english`, English/Arabic only, voices: troy/hannah/austin, at `https://api.groq.com/openai/v1`). Enabled when `tts.api_key` is set in config. Default voice gender is female (`nova` for OpenAI, `troy` for Groq), configurable via `voice_gender` (OpenAI) or `voice` (direct).
